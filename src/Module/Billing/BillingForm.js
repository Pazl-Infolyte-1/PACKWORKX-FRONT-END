...
useEffect(() => {
    const fetchBill = async () => {
      ...
      try {
        ...
        console.log('Bill Details by ID:', response?.data)
      } catch (error) {
  ...
  console.log('Received bill from state:', JSON.stringify(bill))
  const navigate = useNavigate()
useEffect(() => {
    const fetchPurchaseOrders = async () => {
      const response = await purchaseOrderApi.getPurchaseOrderDropdown()
      console.log("pooo",JSON.stringify(response?.data))
...
useEffect(() => {
  if (bill) {
    ...
    setValue('bill_date', bill.bill_date || '');
...
const allPurchaseOrders = useMemo(() => {
  ...
  if (bill?.purchaseOrder?.id && 
      !purchaseOrders.some(po => po.id === bill.purchaseOrder.id)) {
    options.unshift({
...
>
  ...
  {allPurchaseOrders.map((po) => (
    ...
    <div>
      <label className="block text-sm font-medium text-gray-700 mb-1">Bill Reference Number</label>
      <input
        type="text"
