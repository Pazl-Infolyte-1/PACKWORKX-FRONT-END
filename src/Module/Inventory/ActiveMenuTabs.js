...
export const ProductDetails = ({ itemDetails, item, navigate, handleEdit }) => {
  ...
  const handleDownloadQR = async (url) => {
    try {
      ...
      link.download = `product-qr-${itemDetails?.products?.item_generate_id || 'code'}.png`
      ...
      {itemDetails?.products ? (
        <div className="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden mt-1">
          ...
          <div className="p-4 space-y-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">
                      {itemDetails.products.custom_fields.uom || 'N/A'}
                    </p>
                  </div>
                </div>
              </div>
            ...
            {console.log('Item Details qr code:', itemDetails?.products?.qr_code_url)}

            <div className="grid grid-cols-3 gap-4">
              {/* First Column: Your QR Code Box */}
              <div className="overflow-hidden bg-white border border-gray-200 rounded-lg shadow-sm">
                <div className="p-3 border-b border-gray-200">
                  <h3 className="text-sm font-medium text-gray-700">Product QR Code</h3>
                </div>
                <div className="flex flex-col items-center p-4">
                  {itemDetails?.products?.qr_code_url ||
                  'https://imgs.search.brave.com/znAUNdoz16sc9KdnG_yAXIp1PbojCOj3klSxYEmJySw/rs:fit:860:0:0:0/g:ce/aHR0cHM6Ly9tZWRp/YS5zbmwubm8vbWVk/aWEvMTk0Nzc2L3N0/YW5kYXJkX3FyLWtv/ZGUucG5n' ? (
                    <>
                      <img
                        src={itemDetails?.products?.qr_code_url}
                        alt="Product Details QR Code"
                        className="w-48 h-48 object-contain mb-3"
                      />
                      <button
                        onClick={() => handleDownloadQR(itemDetails?.products?.qr_code_url)}
