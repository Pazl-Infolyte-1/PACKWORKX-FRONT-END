      ...
      <ul className="divide-y divide-gray-200">
        {items.map((item) => (
          <li key={item.id} className="px-4 py-2.5 hover:bg-gray-50 transition-colors">
            {editingId === item.id ? (
              <EditingOption item={item} onFinish={onFinishEdit} onCancel={() => onCancelEdit()} />
            ) : (
              <DisplayOption
                item={item}
...
const EditingOption = ({ item, onFinish, onCancel }) => {
  const [editValue, setEditValue] = useState(item.dropdown_value)
  const handleSave = () => {
    onFinish(item, editValue)
  }
  console.log(item)
  ...
  return (
    <div className="flex items-center gap-2">
      <input
        ...
        onKeyDown={(e) => {
          if (e.key === 'Enter') onFinish(item.id, editValue)
...
const DisplayOption = ({ item, onEdit, onDelete }) => {
  return (
    <div className="flex justify-between items-center">
      <span className="text-gray-800 text-sm">{item.dropdown_value}</span>
