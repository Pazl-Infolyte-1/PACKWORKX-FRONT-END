...
const StockOverview = ({ tableData }) => {
  ...
  <div className="flex justify-between py-1">
    ...
    <span className="font-medium">{tableData?.created_by || '-'}</span>
  </div>
    <div className="flex justify-between py-1">
    <span className="text-gray-600">Updated By</span>
    <span className="font-medium">{tableData?.updated_by || '-'}</span>
  </div>
  </div>
			  </div>
		  )}
        ...
        <tbody>
          {tableData.StockAdjustmentItems.map((item) => (
            <tr key={item.id} className="text-center">
              ...
              <td className="px-4 py-2 border border-gray-300">{item.adjustment_quantity}</td>
