...
import { cilPencil, cilPhone, cilSettings } from "@coreui/icons";
      ...
      <div className="border-b border-gray-300 mt-2"></div> {/* Thinner horizontal line */}
        ...
        <div className="ml-3"> {/* Reduced margin */}
         {/*<p className="font-semibold text-sm m-0">Mr. Siva Shakthi Ram</p>*/}
		   <p className="font-semibold text-sm m-0">{tableData?.salutation}{tableData?.first_name}{tableData?.last_name}</p>
...
<h2 className="font-normal text-[13px] text-gray-800">ADDRESS</h2>
        ...
        {/* Arrow Icon */}
        <span
          className={`text-[#438efc] text-xs transform transition-transform duration-300 ${
  ...
  <div>
    ...
    <div className="relative">
      ...
      <CIcon
        icon={cilPencil}
        className="text-gray-500 cursor-pointer absolute right-0 top-1/2 transform -translate-y-1/2 text-xs"
      />
    </div>
    ...
    <p className="m-0">{tableData?.addresses[0]?.city}</p>
	  <p className="m-0">{tableData?.addresses[0]?.state}-{tableData?.addresses[0]?.pincode}</p>
...
<div className="py-1 text-sm text-gray-700 relative">
  <div>
    ...
    <p className="m-0">{tableData?.addresses[1]?.city}</p>
	  <p className="m-0">{tableData?.addresses[1]?.state}-{tableData?.addresses[1]?.pincode}</p>
...
<div className="py-1 text-sm text-gray-700 relative">
 <p className="text-xs m-0 text-[#3e8efd]">
...
<h2 className="font-normal text-[13px] text-gray-800">CONTACT PERSONS</h2>
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v6m3-3H9m12 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
        ...
        <span
          className={`text-[#438efc] text-xs transform transition-transform duration-300 ${
            isOpenContact ? 'rotate-180' : 'rotate-0'
          }`}
        >
          ▼
        </span>
...
<h2 className="font-normal text-[13px] text-gray-800">RECORD INFO</h2>
        <span
          className={`text-[#438efc] text-xs transform transition-transform duration-300 ${
            isOpenRecord ? 'rotate-180' : 'rotate-0'
          }`}
        >
          ▼
        </span>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td className="py-2 font-medium">{moment(tableData?.created_at).format('DD/MM/YYYY')}</td>
    </tr>
    <tr>
...
<div className="mt-4">
    ...
    <table className="min-w-full table-auto border border-gray-200">
      <thead className="bg-gray-100">
        <tr>
          ...
          <th className="px-4 py-2 text-left text-sm font-medium text-gray-700">Outstanding Receivables</th>
          <th className="px-4 py-2 text-left text-sm font-medium text-gray-700">Unused Credits</th>
        </tr>
      </thead>
  ...
  <p className="text-sm m-0 text-[#3e8efd] mt-2">
