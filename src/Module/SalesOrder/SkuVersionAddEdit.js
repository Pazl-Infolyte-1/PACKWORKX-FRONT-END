...
function SkuVersionAddEdit({ skuID, setSkuVersionsMap, orderId  }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // After successful creation, fetch the updated list of versions
      const updatedVersionsResponse = await apiMethods.getSkuVersions(skuID);
      const skuversionID = `V${updatedVersionsResponse.data.data.length + 1}.0`; // Convert to "V1.0", "V2.0", etc.
      setSkuVersion(skuversionID)
