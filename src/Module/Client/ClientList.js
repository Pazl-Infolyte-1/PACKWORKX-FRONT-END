...
function ClientList() {
   ...
   const [commonField,setCommonField] = useState([])
    ...
    const [rows, setRows] = useState([
        ...
        const addRow = () => {
          setRows([...rows, { salutation: "", firstName: "", lastName: "", email: "", workPhone: "", mobile: "" }]);
        };
        useEffect(() => {
  ...
  return <div>
<button className="h-10 flex items-center font-bold bg-[#21338e] text-white px-4 py-2 rounded-lg shadow-md border-none cursor-pointer ml-auto"
      ...
      input.fieldId === 22 ? (
        <div key={input.id} className="flex items-center mb-4">
          ...
          <select className="w-full border border-gray-300 p-2 rounded ml-10">
            {input.options?.map((option) => (
              <option key={option.id} value={option.id} selected={option.id.toString() === input.defaultValue}>
                {option.label}
              </option>
            ))}
          </select>
...
{dynamicFields?.data?.sections
  ...
  dynamicFields?.data?.sections
    ...
    ?.map((section) => (
      <div key={section.groupId} className="p-4 bg-white shadow-md rounded-lg w-[200%] ml-5">
