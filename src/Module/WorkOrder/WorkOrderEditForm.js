...
function WorkOrderEditForm({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filteredSkus = allSkus.filter((skuItem) => allowedSkuNames.includes(skuItem.sku_name))
  ...
  const formatDate = (dateStr) => {
    ...
    // Match DD-MM-YYYY and ignore time part
  ...
  const validateForm = (formData) => {
    const requiredFields = ['sku_id', 'planned_start_date', 'planned_end_date', 'edd', 'qty']
  ...
  const handleSubmit = async (e) => {
    ...
    // Format date fields before validation and submission
    const payload = {
      ...formValues,
      planned_start_date: formatDateForPayload(formValues.planned_start_date),
      planned_end_date: formatDateForPayload(formValues.planned_end_date),
      edd: formatDateForPayload(formValues.edd),
    }
    ...
    try {
      await workOrderApi.editWorkOrder(payload.id, payload)
