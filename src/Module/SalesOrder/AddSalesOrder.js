...
const AddSalesOrder = ({ currentTab,isEdit,selectedSalesOrderID }) => {
  const [activeTab, setActiveTab] = useState(currentTab)
  const [loading, setLoading] = useState(false)
  const [existingSalesOrderData,setExistingSalesOrderData] = useState('')
  const [alerts,setAlerts]=useState([])
  const [skuDetailsForm, setSkuDetailsForm] = useState([]); // New state for SKU details
  ...
  useEffect(() => {
    console.log('salesDetailsForm changed:', salesDetailsForm);
  }, [salesDetailsForm]);
  ...
  const fetchSalesOrderData = async () => {
    if (isEdit && selectedSalesOrderID) {
      ...
      try {
        ...
        console.log(response.data,'Bro')
        setSalesDetailsForm((prev) => {
          const updatedDetails = {
            ...prev, // Keep previous structure
            ...response.data, // Merge new data from API
          };
          return updatedDetails;
        });
        setSkuDetailsForm(response.data.SalesSkuDetails || []); // Update SKU details
    ...
    <div className="screen p-4">
      ...
      <div className="bg-white">
        {activeTab === 'salesOrder' && (
          <OrderForm formData={salesDetailsForm} setFormData={setSalesDetailsForm} skuDetailsForm={skuDetailsForm} />
        )}
